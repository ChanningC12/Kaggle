...
# Assumption: For team that did not have seed in the previous year, assign 20 to Seed_Diff
Seed$Seed_Pre[is.na(Seed$Seed_Pre)] = 20
Seed$Seed_Diff = Seed$Seed_Pre - Seed$Seed_num

# Output Seed table
write.csv(Seed,"Intermediate Data/Seed.csv",row.names = F)

############## Pre-metric creation Process ##################
rm(list=ls())
gc()
...
Game$numot_qc <- Game$Numot.x == Game$Numot.y
...
gc()
# Read in Test Data
Test_Full = read.csv("Raw Data/SampleSubmission.csv") # read in submission dataset
...
# Merge in seed information to the submission file
...
# Merge in metric dataset to the submission file
...
# Create metric differential variables
...
unlist(Tournament_auc@y.values) # 0.752
...
write.csv(Pred_Tournament_Top,"Pred_Tournament_Top_xgb_Elo_0313.csv",row.names = F)
