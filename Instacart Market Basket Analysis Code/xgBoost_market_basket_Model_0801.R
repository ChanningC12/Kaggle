...
train.1 <- train[train$cluster==1,] %>% select(-cluster)
train.2 <- train[train$cluster==2,] %>% select(-cluster)
train.3 <- train[train$cluster==3,] %>% select(-cluster)
train.4 <- train[train$cluster==4,] %>% select(-cluster)
train.5 <- train[train$cluster==5,] %>% select(-cluster)
...
model.1 <- xgboost(data = X.trn.1, params = params, nrounds = 100) # logloss: 0.249786
...
write.csv(importance,"../Model Output/080101/variable_importance_xgb_080101_clust1.csv",row.names = F)
...
names(f_score) <- c("x","y")
f_score[f_score$y==max(f_score[!(is.na(f_score$y)),]$y),] # 0.2182838 0.4674714
abline(v = 0.2182838,col="red",lty=2)
...
write.csv(importance,"../Model Output/080101/variable_importance_xgb_080101_clust2.csv",row.names = F)
xgb.ggplot.importance(importance)
...
write.csv(importance,"../Model Output/080101/variable_importance_xgb_080101_clust3.csv",row.names = F)
xgb.ggplot.importance(importance)
...
write.csv(importance,"../Model Output/080101/variable_importance_xgb_080101_clust4.csv",row.names = F)
xgb.ggplot.importance(importance)
...
write.csv(importance,"../Model Output/080101/variable_importance_xgb_080101_clust5.csv",row.names = F)
xgb.ggplot.importance(importance)
